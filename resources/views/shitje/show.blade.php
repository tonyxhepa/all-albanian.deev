@extends('layouts.app')
@section('header')
    <title>{{ $shiko_shitjet->title }}</title>
    ...
    <div class="container">
        <div class="row">
            <div class="col-xs-12 col-sm-6 col-md-9">
                <div class="row">
                     <div class="pull-right">Publikuar: {{ $shiko_shitjet->created_at->format('d M Y - H:i:s') }}</div>
                </div>
                <div class="row">
                    @if(count($shiko_shitjet->photos) >=1)
                        @foreach($shiko_shitjet->photos as $photo)
                            <div class="thumbnails-tony">
                                <a href="{{ asset('storage').$photo->thumbnail }}" data-lightbox="gallery" data-title="{{ $shiko_shitjet->title }}">
                ...
                <div class="">
                    <h3> {{ $shiko_shitjet->title }}</h3>
                    <div class="">{{ $shiko_shitjet->pershkrimi }}</div>
                </div>
            </div>
