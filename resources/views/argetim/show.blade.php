    ...
    <div class="container">
        <div class="row">
            <div class="col-xs-12 col-sm-6 col-md-9">
                <div class="row">
                     <div class="pull-right">Publikuar: {{ $shiko_argetimin->created_at->format('d M Y - H:i:s') }}</div>
                </div>
                <div class="row">
                    @if(count($shiko_argetimin->photos) >=1)
                        @foreach($shiko_argetimin->photos as $photo)
                            <div class="thumbnails-tony">
                                <a href="{{ asset('storage').$photo->thumbnail }}" data-lightbox="gallery" data-title="{{ $shiko_argetimin->title }}">
