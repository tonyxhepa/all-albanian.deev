@extends('layouts.app')
@section('header')
    <title>{{ $shiko_sportin->title }}</title>
    ...
    <div class="container">
        <div class="row">
            <div class="col-xs-24 col-sm-12 col-md-18">
                <div class="row">
                     <div class="pull-right">Publikuar: {{ $shiko_sportin->created_at->format('d M Y - H:i:s') }}</div>
                </div>
                <div class="row">
                    @if(count($shiko_sportin->photos) >=1)
                        <div id="aniimated-thumbnials">
                            @foreach($shiko_sportin->photos as $photo)
                                    <a class="col-sm-6" href="{{ asset('storage').$photo->thumbnail }}" data-lightbox="gallery" data-title="{{ $shiko_sportin->title }}">
                ...
                <div class="">
                    <h3> {{ $shiko_sportin->title }}</h3>
                    <div class="">{{ $shiko_sportin->pershkrimi }}</div>
                </div>
            </div>
