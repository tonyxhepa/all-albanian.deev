@extends('layouts.app')
@section('header')
    <title>{{ $shiko_pronat->title }}</title>
    ...
    <div class="container">
        <div class="row">
            <div class="col-xs-12 col-sm-6 col-md-9">
                <div class="row">
                     <div class="pull-right">Publikuar: {{ $shiko_pronat->created_at->format('d M Y - H:i:s') }}</div>
                </div>
                <div class="row">
                    @if(count($shiko_pronat->photos) >=1)
                        @foreach($shiko_pronat->photos as $photo)
                            <div class="thumbnails-tony">
                                <a href="{{ asset('storage').$photo->thumbnail }}" data-lightbox="gallery" data-title="{{ $shiko_pronat->title }}">
                ...
                <div class="">
                    <h3> {{ $shiko_pronat->title }}</h3>
                    <div class="">{{ $shiko_pronat->pershkrimi }}</div>
                </div>
            </div>
